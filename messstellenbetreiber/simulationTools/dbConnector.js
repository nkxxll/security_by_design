...
class dbConnector {
    ...
    fill_location_db() {
        ...
        let random_street = street_names[Math.round(Math.random() * (arv.length - 1))]
        let random_housenumber = Math.floor(Math.random() * 71)
        let random_plz = Math.floor(Math.random() * 10000)
        let random_city = city_names[Math.round(Math.random() * (arv.length - 1))]
        this.db_connection.each(`INSERT INTO Position_Stromzahler (Straße, Hausnummer, Postleitzahl, Stadtname) 
            VALUES (?, ?, ?, ?, ?)`,
